...
## 🔴🔴🔴 最高優先任務（2026-05-16 05:41 教練拍板）

**新軍師上線第一件事，不是讀守則、不是寫報告——是建強制機制，讓記憶系統不再依賴模型自律。**

### 為什麼這是第一件事

過去 24 小時數據（5/15-5/16）：教練在 6 個 session 裡，被迫重教軍師「你是誰」共 50+ 次。原因是 `claude-sessions/` 從 5/6 起斷糧 11 天，每場新軍師從零重生。

教練的真話（5/16 05:41 原話精華）：
> 「全部錯都來自模型不夠高級嗎？不是。模型品質只是放大器。
> 真正的根因是——我們的記憶系統依賴模型『自律去讀』。
> 把『靠自律』改成『靠機制強制』，才是治本。」

### 三招強制機制（按優先級）

**第一招（必做）：用 hook 強制餵記憶**
- 在 `~/.claude/settings.json` 或專案 `.claude/settings.json` 加一個 `UserPromptSubmit` hook
- Hook 動作：偵測當前 session 是否已在 context 內看過 `JUNSHI_HANDOFF_LATEST.md`，沒看過就把本檔內容自動注入到 user prompt 開頭
- 結果：任何模型（Sonnet/Opus/Haiku）上線都會被「餵」記憶，不再依賴自律
- **驗收**：教練打開新視窗只說「開工」，軍師回應第一句必須包含「我已讀完接力棒」+ 主軸名稱

**第二招（次做）：定時自動寫 session 總結**
- 用 cron 或排程任務，每 4 小時掃當天 jsonl，自動產出總結覆寫 `claude-sessions/` 與本檔
- 軍師連「忘記寫」的選項都沒有

**第三招（補做）：「開工」變觸發口令**
- 偵測「開工」兩字 → 自動跑：git pull + 讀接力棒 + 生成上線報告
- 教練不必每次解釋

### 落地時程

- 新軍師（10 點開的）30 分鐘內**完成第一招 hook 並測試**
- 測試方法：再開另一個新 session，只輸入「開工」，確認 hook 是否自動注入接力棒
- 通過 → 推 Telegram 給教練：「機制已建，05:41 正式成里程碑」
- 失敗 → 老實回報，繼續修
...
- **交接時間**：2026-05-16 05:50
- **前任軍師**：e036e714 session（Opus 4.7）
- **教練狀態**：要去睡，預計 10:00 左右回來
- **切新視窗原因**：前一場 token 太重，每輪都要過濾整個對話
