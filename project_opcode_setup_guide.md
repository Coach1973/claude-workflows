...
## 📖 背景說明
教練在跨平臺（Mac/Windows 聯想電腦）安裝開源軟體 Opcode (語音辨識為 OPUD) 後，必須配置對應的 API 才能順利運行。為了讓未來的部署（每臺電腦）都能快速複製貼上、無腦設定，特此記錄這份標準設定流程。此配置使用了代理伺服器 (easyclaude.com) 來無縫接入 Anthropic 服務。
